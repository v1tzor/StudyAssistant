 ...
 */
@Composable
fun TodoWidgetContent(state: TodoWidgetStateUi) {
    val context = LocalContext.current
    val sizeClass = WidgetSizeClass.fetch(LocalSize.current)
    val listDestination = WidgetDeepLinkDestination.Todos

    WidgetScaffold(
        header = {
            WidgetHeader(
                title = widgetString(R.string.widget_todos_title),
                ...
                actionDescription = widgetString(R.string.widget_add_todo_description),
                action = actionStartActivity(
                    WidgetDeepLinkFactory.createIntent(
                        context,
                        WidgetDeepLinkDestination.TodoEditor(todoId = null),
                    ),
                ),
        ...
        WidgetStateContent(
            ...
            emptyTitle = widgetString(R.string.widget_todos_empty),
        ) {
            LazyColumn(GlanceModifier.fillMaxSize()) {
                items(state.items, itemId = { it.id }) { todo ->
                    Box(GlanceModifier.padding(bottom = WidgetDimensions.spacingSmall)) {
                        TodoWidgetRow(
                            ...
                            action = actionStartActivity(
                                WidgetDeepLinkFactory.createIntent(
                                    context,
                                    WidgetDeepLinkDestination.TodoEditor(todo.todoId),
                                ),
                            ),
                        )
                    }
