 ...
 */
@Composable
fun ScheduleWidgetContent(state: ScheduleWidgetStateUi) {
    val context = LocalContext.current
    val sizeClass = WidgetSizeClass.fetch(LocalSize.current)
    val date = state.date.takeIf { it > 0L } ?: System.currentTimeMillis()
    val scheduleDestination = WidgetDeepLinkDestination.Schedule

    WidgetScaffold(
        header = {
            WidgetHeader(
                title = widgetString(R.string.widget_schedule_title),
                titleAction = actionStartActivity(
                    WidgetDeepLinkFactory.createIntent(context, scheduleDestination),
                ),
                actionIcon = ImageProvider(R.drawable.ic_widget_add),
                actionDescription = widgetString(R.string.widget_add_schedule_description),
                action = actionStartActivity(
                    WidgetDeepLinkFactory.createIntent(
                        context = context,
                        destination = WidgetDeepLinkDestination.ScheduleEditor(
                            date = date,
                            customScheduleId = state.customScheduleId,
                            baseScheduleId = state.baseScheduleId,
                        ),
                    ),
                ),
        ...
        WidgetStateContent(
            ...
            emptyTitle = widgetString(R.string.widget_schedule_empty),
        ) {
            LazyColumn(GlanceModifier.fillMaxSize()) {
                items(state.items, itemId = { it.id }) { item ->
