 ...
 */
internal fun AnalyticsLoadDistribution.mapToUi() = AnalyticsLoadDistributionUi(
    buckets = buckets.map { it.mapToUi() },
    averageWorkload = averageWorkload,
    peakBucket = peakBucket?.mapToUi(),
    daysAboveThreshold = daysAboveThreshold,
    threshold = threshold,
)

internal fun AnalyticsLoadBucket.mapToUi() = AnalyticsLoadBucketUi(
    from = from,
    to = to,
    workload = workload,
