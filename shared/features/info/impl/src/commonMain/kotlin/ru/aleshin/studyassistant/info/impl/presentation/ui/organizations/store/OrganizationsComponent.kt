 ...
 */
internal abstract class OrganizationsComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: OrganizationsComposeStore

    class Default(
        storeFactory: OrganizationsComposeStore.Factory,
        componentContext: ComponentContext,
        outputConsumer: OutputConsumer<OrganizationsOutput>,
    ) : OrganizationsComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "INFO_ORGANIZATIONS"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = OrganizationsState(),
            stateSerializer = OrganizationsState.serializer(),
