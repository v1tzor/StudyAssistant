 ...
 */
internal abstract class OrganizationComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: OrganizationComposeStore

    class Default(
        storeFactory: OrganizationComposeStore.Factory,
        componentContext: ComponentContext,
        inputData: OrganizationInput,
        outputConsumer: OutputConsumer<OrganizationOutput>,
    ) : OrganizationComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "EDITOR_ORGANIZATION"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = OrganizationState(),
            stateSerializer = OrganizationState.serializer(),
            input = inputData,
