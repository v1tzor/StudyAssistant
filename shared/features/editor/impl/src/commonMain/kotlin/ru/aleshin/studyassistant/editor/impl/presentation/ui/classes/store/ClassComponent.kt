 ...
 */
internal abstract class ClassComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: ClassComposeStore

    class Default(
        storeFactory: ClassComposeStore.Factory,
        componentContext: ComponentContext,
        inputData: ClassInput,
        outputConsumer: OutputConsumer<ClassOutput>,
    ) : ClassComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "EDITOR_CLASS"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = ClassState(),
            stateSerializer = ClassState.serializer(),
            input = inputData,
