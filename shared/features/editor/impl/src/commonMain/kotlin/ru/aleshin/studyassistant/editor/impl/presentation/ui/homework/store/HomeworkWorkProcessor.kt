...
internal interface HomeworkWorkProcessor :
    FlowWorkProcessor<HomeworkWorkCommand, HomeworkAction, HomeworkEffect, HomeworkOutput> {
    ...
    class Base(
        ...
        private fun loadEditModelWork(
            ...
            val showDeleteAction = homework != null
            ...
            emit(ActionResult(HomeworkAction.SetupEditModel(editModel, showDeleteAction)))
        }
        ...
        private fun deleteHomeworkWork(editModel: EditHomeworkUi) = flow {
            homeworkInteractor.deleteHomework(editModel.uid).handle(
                onLeftAction = { emit(EffectResult(HomeworkEffect.ShowError(it))) },
                onRightAction = { emit(OutputResult(HomeworkOutput.NavigateToBack)) },
            )
        }
...
internal typealias HomeworkWorkResult = WorkResult<HomeworkAction, HomeworkEffect, HomeworkOutput>