...
internal interface DailyScheduleWorkProcessor :
    FlowWorkProcessor<DailyScheduleWorkCommand, DailyScheduleAction, DailyScheduleEffect, DailyScheduleOutput> {
    ...
    class Base(
        ...
        private fun loadSchedulesWork(baseScheduleId: UID?, customScheduleId: UID?) = flow {
            ...
            customScheduleFlow.combineWithResult(
        ...
        private fun deleteCustomScheduleWork(customScheduleId: UID) = flow {
            customScheduleInteractor.deleteScheduleById(customScheduleId).handle(
                onLeftAction = { emit(EffectResult(DailyScheduleEffect.ShowError(it))) },
                onRightAction = { emit(OutputResult(DailyScheduleOutput.NavigateToBack)) },
            )
        }
...
internal typealias DailyScheduleWorkResult = WorkResult<DailyScheduleAction, DailyScheduleEffect, DailyScheduleOutput>