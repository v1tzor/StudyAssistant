...
internal abstract class EmployeeComponent(
    ...
    abstract val store: EmployeeComposeStore

    class Default(
        storeFactory: EmployeeComposeStore.Factory,
        componentContext: ComponentContext,
        inputData: EmployeeInput,
        outputConsumer: OutputConsumer<EmployeeOutput>,
    ) : EmployeeComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "EDITOR_EMPLOYEE"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = EmployeeState(),
            stateSerializer = EmployeeState.serializer(),
            input = inputData,
