 ...
 */
internal abstract class WeekScheduleComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: WeekScheduleComposeStore

    class Default(
        storeFactory: WeekScheduleComposeStore.Factory,
        componentContext: ComponentContext,
        inputData: WeekScheduleInput,
        outputConsumer: OutputConsumer<WeekScheduleOutput>,
    ) : WeekScheduleComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "EDITOR_WEEK_SCHEDULE"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = WeekScheduleState(),
            stateSerializer = WeekScheduleState.serializer(),
            input = inputData,
