...
import ru.aleshin.studyassistant.editor.impl.presentation.ui.schedule.screenmodel.rememberWeekScheduleScreenModel
 ...
 */
internal data class WeekScheduleScreen(val week: NumberOfRepeatWeek) : Screen {

    @Composable
    @OptIn(ExperimentalMaterial3Api::class)
    override fun Content() = ScreenContent(
        screenModel = rememberWeekScheduleScreenModel(),
        initialState = WeekScheduleViewState(),
            ...
            BottomSheetScaffold(
                ...
                sheetContent = {
                    WeekScheduleBottomSheet(
                        ...
                        onSaveClick = { dispatchEvent(WeekScheduleEvent.NavigateToBack) },
                        onUpdateOrganization = { dispatchEvent(WeekScheduleEvent.UpdateOrganization(it)) },
                        onAddOrganization = { dispatchEvent(WeekScheduleEvent.NavigateToOrganizationEditor) },
                        onSelectedWeek = { dispatchEvent(WeekScheduleEvent.ChangeWeek(it)) },
                    )
                },
                content = { paddingValues ->
                    WeekScheduleContent(
                        ...
                        onRefresh = { dispatchEvent(WeekScheduleEvent.Refresh) },
                        onCreateClass = { weekDay, schedule ->
                            dispatchEvent(WeekScheduleEvent.CreateClassInEditor(weekDay, schedule))
                        },
                        onEditClass = { editClass, weekDay ->
                            dispatchEvent(WeekScheduleEvent.EditClassInEditor(editClass, weekDay))
                        },
                        onDeleteClass = { uid, schedule ->
                            dispatchEvent(WeekScheduleEvent.DeleteClass(uid, schedule))
                        },
                    )
                },
                topBar = {
        ...
        handleEffect { effect ->
            when (effect) {
                is WeekScheduleEffect.NavigateToBack -> navigator.nestedPop()
