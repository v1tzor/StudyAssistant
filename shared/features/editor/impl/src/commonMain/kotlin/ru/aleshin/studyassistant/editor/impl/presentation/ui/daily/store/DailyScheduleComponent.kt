 ...
 */
internal abstract class DailyScheduleComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: DailyScheduleComposeStore

    class Default(
        storeFactory: DailyScheduleComposeStore.Factory,
        componentContext: ComponentContext,
        inputData: DailyScheduleInput,
        outputConsumer: OutputConsumer<DailyScheduleOutput>,
    ) : DailyScheduleComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "EDITOR_DAILY_SCHEDULE"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = DailyScheduleState(),
            stateSerializer = DailyScheduleState.serializer(),
            input = inputData,
