 ...
 */
internal abstract class HomeworkComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: HomeworkComposeStore

    class Default(
        storeFactory: HomeworkComposeStore.Factory,
        componentContext: ComponentContext,
        inputData: HomeworkInput,
        outputConsumer: OutputConsumer<HomeworkOutput>,
    ) : HomeworkComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "EDITOR_HOMEWORK"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = HomeworkState(),
            stateSerializer = HomeworkState.serializer(),
            input = inputData,
