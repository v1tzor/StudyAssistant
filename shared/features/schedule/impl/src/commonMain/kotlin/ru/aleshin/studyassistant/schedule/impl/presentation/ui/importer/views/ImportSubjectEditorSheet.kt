...
internal fun ImportSubjectEditorSheet(
    ...
    val linked = subjects.firstOrNull { subject -> subject.name.equals(subjectName, ignoreCase = true) }
    var subjectId by remember { mutableStateOf(linked?.uid) }
    ...
    ModalBottomSheet(
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                onValueChange = { value ->
                    name = value
                    subjectId = subjects.firstOrNull { subject ->
                        subject.name.equals(value, ignoreCase = true)
                    }?.uid
                },
            ...
            Button(
                ...
                onClick = {
                    onConfirm(
                        entries.map { entry ->
                            if (entry.subject.equals(subjectName, ignoreCase = true)) {
                                entry.copy(subject = name, subjectId = subjectId)
                            } else {
                                entry
                            }
                        },
