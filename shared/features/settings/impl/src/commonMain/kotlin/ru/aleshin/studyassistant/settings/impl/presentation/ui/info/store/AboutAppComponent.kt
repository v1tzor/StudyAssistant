 ...
 */
internal abstract class AboutAppComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: AboutAppComposeStore

    class Default(
        storeFactory: AboutAppComposeStore.Factory,
        componentContext: ComponentContext,
    ) : AboutAppComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "SETTINGS_ABOUT_APP"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = AboutAppState.Default,
            stateSerializer = AboutAppState.serializer(),
