 ...
 */
internal abstract class InternalAuthFeatureComponent(
    ...
    class Default(
    ...
    ) : InternalAuthFeatureComponent(
        componentContext = componentContext,
        startConfig = startConfig,
        outputConsumer = outputConsumer,
    ) {
        override val contentProvider = AuthContentProvider(this)

        private val backCallback = BackCallback { navigateToBack() }

        private val stackNavigation = StackNavigation<AuthConfig>()

        override val stack: Value<ChildStack<*, Child>> = childStack(
            source = stackNavigation,
            serializer = AuthConfig.serializer(),
            initialStack = { startConfig.backstack ?: listOf(AuthConfig.Login) },
            key = STACK_KEY,
        ...
        override fun navigateToBack() {
            stackNavigation.pop { isPop ->
                if (!isPop) outputConsumer.consume(AuthOutput.NavigateToBack)
            }
        }

        override fun onDestroyInstance() {
            AuthFeatureManager.finish()
        }

        private fun createChild(config: AuthConfig, componentContext: ComponentContext): Child {
            return when (config) {
                is AuthConfig.Login -> Child.LoginChild(
                ...
                is AuthConfig.Register -> Child.RegisterChild(
                ...
                is AuthConfig.Forgot -> Child.ForgotChild(
                ...
                is AuthConfig.Verification -> Child.VerificationChild(
        ...
        private fun loginOutputConsumer() = OutputConsumer<LoginOutput> { data ->
            when (data) {
                is LoginOutput.NavigateToSignUp -> {
                    stackNavigation.pushToFront(AuthConfig.Register)
                }
                is LoginOutput.NavigateToRecovery -> {
                    stackNavigation.pushToFront(AuthConfig.Forgot)
                }
                is LoginOutput.NavigateToVerification -> {
                    stackNavigation.replaceAll(AuthConfig.Verification)
                }
                is LoginOutput.NavigateToFirstSetup -> {
                    outputConsumer.consume(AuthOutput.NavigateToFirstSetup)
                }
                is LoginOutput.NavigateToApp -> {
                    outputConsumer.consume(AuthOutput.NavigateToApp)
                }
            }
        }
        ...
        private fun registerOutputConsumer() = OutputConsumer<RegisterOutput> { data ->
            when (data) {
                is RegisterOutput.NavigateToLogin -> {
                    stackNavigation.pushToFront(AuthConfig.Login)
                }
        ...
        private fun verificationOutputConsumer() = OutputConsumer<VerificationOutput> { data ->
            when (data) {
                is VerificationOutput.NavigateToLogin -> {
                    stackNavigation.replaceAll(AuthConfig.Login)
                }
