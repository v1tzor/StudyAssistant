...
import ru.aleshin.studyassistant.auth.api.AuthFeatureApi
 ...
 */
public object AuthFeatureManager : BaseFeatureManager<AuthFeatureApi, AuthFeatureDependencies> {

    private var directDI: DirectDI? = null

    override fun createOrGetFeature(dependencies: AuthFeatureDependencies): AuthFeatureApi {
        val diGraph = directDI

        return if (diGraph != null) {
            diGraph.instance<AuthFeatureApi>()
        } else {
            val di = DI {
                ...
                bindSingleton<AuthFeatureApi> {
                    object : AuthFeatureApi {
                        override fun componentFactory(): AuthFeatureComponentFactory = instance()
                    }
                }
            }
            directDI = di.direct

            di.direct.instance<AuthFeatureApi>()
        }
    }
