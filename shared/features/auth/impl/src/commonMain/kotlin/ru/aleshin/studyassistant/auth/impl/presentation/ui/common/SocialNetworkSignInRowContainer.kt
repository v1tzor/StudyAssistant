 ...
 */
@Composable
internal fun SocialNetworkSignInRowContainer(
    modifier: Modifier = Modifier,
    horizontalArrangement: Arrangement.Horizontal = Arrangement.spacedBy(12.dp),
    onSuccess: (UserSession) -> Unit = {},
    onError: (Exception?) -> Unit = {},
    content: @Composable RowScope.(SocialNetworkSignInCallback) -> Unit,
) {
    Box(modifier = modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
        Row(
            ...
            val successCallback by rememberUpdatedState(onSuccess)
            ...
            content.invoke(this@Row) { provider ->
                coroutineScope.launch {
                    try {
                        ...
                        if (session != null) {
                            successCallback(session.mapToDomain())
                        } else {
                            errorCallback(null)
                        }
                    } catch (e: Exception) {
