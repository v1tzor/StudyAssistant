 ...
 */
internal abstract class IntroComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: IntroComposeStore

    class Default(
        storeFactory: IntroComposeStore.Factory,
        componentContext: ComponentContext,
        outputConsumer: OutputConsumer<IntroOutput>,
    ) : IntroComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "PREVIEW_INTRO"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = IntroState,
            stateSerializer = IntroState.serializer(),
