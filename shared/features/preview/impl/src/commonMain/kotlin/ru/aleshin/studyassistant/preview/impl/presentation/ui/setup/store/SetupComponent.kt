 ...
 */
internal abstract class SetupComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: SetupComposeStore

    class Default(
        storeFactory: SetupComposeStore.Factory,
        componentContext: ComponentContext,
        outputConsumer: OutputConsumer<SetupOutput>,
    ) : SetupComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "PREVIEW_SETUP"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = SetupState(),
            stateSerializer = SetupState.serializer(),
