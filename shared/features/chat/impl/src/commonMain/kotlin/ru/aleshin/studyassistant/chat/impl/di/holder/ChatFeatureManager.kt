 ...
 */
public object ChatFeatureManager : BaseFeatureManager<ChatFeatureApi, ChatFeatureDependencies> {

    private var directDi: DirectDI? = null

    override fun createOrGetFeature(dependencies: ChatFeatureDependencies): ChatFeatureApi {
        val diGraph = directDi

        return if (diGraph != null) {
            diGraph.instance<ChatFeatureApi>()
        } else {
            val di = DI {
                ...
                bindSingleton<ChatFeatureApi> {
                    object : ChatFeatureApi {
                        override fun componentFactory(): ChatFeatureComponentFactory = instance()
                    }
                }
            }
            directDi = di.direct

            di.direct.instance<ChatFeatureApi>()
        }
    }
