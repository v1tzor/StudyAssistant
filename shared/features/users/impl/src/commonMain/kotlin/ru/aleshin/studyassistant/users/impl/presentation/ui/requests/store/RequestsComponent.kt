 ...
 */
internal abstract class RequestsComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: RequestsComposeStore

    class Default(
        storeFactory: RequestsComposeStore.Factory,
        componentContext: ComponentContext,
        outputConsumer: OutputConsumer<RequestsOutput>,
    ) : RequestsComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "USERS_REQUSTS"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = RequestsState(),
            stateSerializer = RequestsState.serializer(),
