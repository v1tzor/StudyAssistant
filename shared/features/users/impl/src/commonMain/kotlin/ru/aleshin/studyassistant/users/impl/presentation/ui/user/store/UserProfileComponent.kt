 ...
 */
internal abstract class UserProfileComponent(
    componentContext: ComponentContext
) : ChildComponent(componentContext) {

    abstract val store: UserProfileComposeStore

    class Default(
        storeFactory: UserProfileComposeStore.Factory,
        componentContext: ComponentContext,
        inputData: UserProfileInput,
        outputConsumer: OutputConsumer<UserProfileOutput>,
    ) : UserProfileComponent(componentContext) {

        private companion object Companion {
            const val COMPONENT_KEY = "USERS_USER"
        }

        override val store by saveableStore(
            storeFactory = storeFactory,
            defaultState = UserProfileState(),
            input = inputData,
            stateSerializer = UserProfileState.serializer(),
