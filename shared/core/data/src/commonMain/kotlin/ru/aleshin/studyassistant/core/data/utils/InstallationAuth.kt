 ...
 */
internal suspend fun <T> InstallationIdProvider.withValidInstallation(
    block: suspend (token: String) -> T,
): T {
    return try {
        block(fetchInstallationId())
    } catch (_: InvalidInstallationException) {
        block(refreshInstallationId())
    }
}
