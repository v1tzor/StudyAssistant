 ...
 */
class AnalyticsSettingsRepositoryImpl(
    private val localDataSource: AnalyticsSettingsLocalDataSource,
) : AnalyticsSettingsRepository {

    override suspend fun fetchSettings(): Flow<AnalyticsSettings> {
        return localDataSource.fetchSettings().map { settings -> settings.mapToDomain() }
    }

    override suspend fun updateSettings(settings: AnalyticsSettings) {
