...
import ru.aleshin.studyassistant.core.data.mappers.billing.mapToDomain
...
class SubscriptionsRepositoryImpl(
    private val subscriptionsApi: SubscriptionsRemoteApi,
) : SubscriptionsRepository {

    override suspend fun fetchSubscriptionsIds(): Flow<List<String>> {
        return subscriptionsApi.fetchSubscriptionsIds().map { productsList ->
            productsList.map { product -> product.id }
        }
    }

    override suspend fun fetchSubscriptionStatus(identifier: SubscriptionIdentifier): SubscriptionStatus? {
        return subscriptionsApi.fetchSubscriptionStatus(identifier)?.mapToDomain()
    }
}