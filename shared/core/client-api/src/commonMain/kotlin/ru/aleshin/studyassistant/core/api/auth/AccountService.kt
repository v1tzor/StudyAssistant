...
class AccountService(
    ...
    fun fetchStateChanged() = channelFlow<AuthUserPojo?> {
        ...
        val accountListener = realtime.subscribe(channels = Channels.account())
            .onEach { event ->
        ...
        val singInListener = signInFlow
            .onEach { send(getCurrentUser(true)) }
            .launchIn(this)
    ...
    suspend fun createOAuth2Session(
        ...
        val apiParams = mutableMapOf(
            ...
            "failure" to failure,
            "scopes" to scopes,
            "project" to AppwriteClient.config["project"],
        )
        ...
        val apiUrl = "${AppwriteClient.endpoint}$apiPath?${apiQuery.joinToString("&")}"
        val callbackUrlScheme = "appwrite-callback-${AppwriteClient.config["project"]}"
    ...
    suspend fun createOAuth2Session(
        ...
        val url = Url(AppwriteClient.endpoint)
        return createOAuth2Session(
            activity = activity,
            provider = provider,
            success = "appwrite-callback-${AppwriteClient.config["project"]}://${url.host}/auth/oauth2/success",
            failure = "appwrite-callback-${AppwriteClient.config["project"]}://${url.host}/auth/oauth2/failure",
            scopes = scopes,
        )
    }
