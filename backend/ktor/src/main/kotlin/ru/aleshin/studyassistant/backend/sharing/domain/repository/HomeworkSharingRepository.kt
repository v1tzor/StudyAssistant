 ...
 */
interface HomeworkSharingRepository {
    suspend fun tryCreate(share: StoredHomeworkShare): CreateHomeworkShareStorageResult
    suspend fun findAvailable(codeHash: ByteArray, now: Instant): StoredHomeworkShare?
    suspend fun recordCodeLookup(installationHash: ByteArray, now: Instant): CodeLookupStorageResult
}